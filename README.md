...
3. Run the following command to delete Old Claud App
   
       sudo apt remove --purge claude-desktop -y
       rm -rf ~/.config/claude*
...
9. Run `uv add "mcp[cli]"` to add mcp cli in your project
10. Few folks may get type errors for which you can run `pip install --upgrade typer` to upgrade typer library to its latest version
11. Write code in main.py for leave management server
12. Install this server inside Claude desktop by running `uv run mcp install main.py` in the project directory
13. Kill any running instance of Claude from Task Manager. Restart Claude Desktop
